...
const Header = (props) => {
  ...
  const search = products.filter((product) => {
    return keys.some((key) => product[key].toLowerCase().includes(writing))
  })
...
////////////////////////////////////////////////////////
  ...
  window.addEventListener("scroll", setHides);
//////////////////////////////////////////////////
  useEffect(()=>{
    const handle = (e) => {
  ...
  return (
    <header className={header ? 'header hide' : 'header'}>
        ...
        <div className="container">
            ...
            <div className={`navbar-mobile ${active ? 'show' : ''}`}>
              ...
              <nav className="navbar">
                <ul>
                  ...
                  <li onClick={()=> setActive(false)}><a onClick={()=>props.sale.current?.scrollIntoView()}>On Sale</a></li>
                  <li onClick={()=> setActive(false)}><a onClick={()=>props.ariwals.current?.scrollIntoView()}>New Arrivals</a></li>
          ...
          <nav className="navbar">
            <ul>
              ...
              <li><a onClick={()=>props.sale.current?.scrollIntoView()}>On Sale</a></li>
              <li><a onClick={()=>props.ariwals.current?.scrollIntoView()}>New Arrivals</a></li>
              <li><a href="">Brands</a></li>
            </ul>
          </nav>
          <div className="search-box">
            <input ref={ref} type="text" placeholder='Search for product...' onChange={(e) => setWriting(e.target.value)}/>
            <div className={`dataResult ${writing ? 'active' : ''}`} onClick={() => reloadPage()} >
              {writing.length != 0 && search.map((product) => (
                  <Link to={`/products/${product.id}`} className='cover-box'>
                    <div className="box">
                      <a href="" className="dataItem">
                        ...
                        <div className="info">
                          ...
                          <div className="rating">
                            <ListStars prod={product}></ListStars>
                            <p>{product.half_rate}/<span>5</span></p>
                          </div>
